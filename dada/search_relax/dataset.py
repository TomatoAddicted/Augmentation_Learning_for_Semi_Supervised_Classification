...
from dada.search_relax.imagenet import ImageNet
from dada.search_relax.operation import Lighting
import os
import numpy as np
...
class AugmentDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        if self.search:
            ...
            if self.ssl:
                weak = self.after_transforms(self.pre_transforms(img))
...
def get_dataloaders_dada(args, labeled_train_set,  labeled_val_set,
                    labeled_train_sampler, val_sampler, ops_names, magnitudes):
    ...
    trainloader = torch.utils.data.DataLoader(
        ...
        sampler=labeled_train_sampler, drop_last=False,
