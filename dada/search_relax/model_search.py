...
from dada.search_relax.operation import apply_augment
from dada.search_relax.networks import get_model
    ...
    def forward(self, origin_images, probability_b, magnitude):
        ...
        for i in range(len(self.sub_policy)):
            if probability_b[i].item() != 0.0:
                #print("Differentiated:", self.sub_policy[i])
...
class Network(nn.Module):
    def __init__(
        ...
        self.create_model_funct = create_model_funct
        self.model = self.create_model(log=log)
    ...
    def create_model(self, log=False):
        return self.create_model_funct(self.args, log=log)
        #return get_model(
        #    self.model_name,
        #    self.num_classes,
        #    self.use_cuda,
    ...
    def _initialize_augment_parameters(self):
        ...
        if self.use_cuda:
            #self.probabilities = Variable(1.*torch.ones(num_sub_policies, num_ops).to(self.device), requires_grad=True)
            if self.args.dada_fix_p:  # set fix values to 1
                self.probabilities = Variable(1.*torch.ones(num_sub_policies, num_ops).to(self.device), requires_grad=True)
            else:  # init values as 0.5
                self.probabilities = Variable(0.5*torch.ones(num_sub_policies, num_ops).to(self.device), requires_grad=True)
            self.ops_weights = Variable(1e-3*torch.ones(num_sub_policies).to(self.device), requires_grad=True)
            self.q_func = [QFunc(num_sub_policies*(num_ops+1)).to(self.device)]
            self.magnitudes = Variable(0.5*torch.ones(num_sub_policies, num_ops).to(self.device), requires_grad=True)
    ...
    def sample(self):
        ...
        probabilities_u = torch.rand(num_sub_policies, num_ops).to(self.device)
        probabilities_v = torch.rand(num_sub_policies, num_ops).to(self.device)
        ...
        ops_weights_u = torch.rand(num_sub_policies).to(self.device)
        ops_weights_v = torch.rand(num_sub_policies).to(self.device)
    ...
    def forward_train(self, origin_images):
        # TODO: hier (oder in extra Funktion) loss für FixMatch hinzufügen (evtl mit if Abfrage)
    ...
    def forward(self, origin_images):
        # TODO: Struktur überdenken: hier wird nur augmenting abgefragt, es sollte auch labeled/unlabeled abgefragt werden(?)
        if self.augmenting:
            # TODO: hier evtl nochmal nach fixmatch/supervised abfragen und separate Funktionsaufrufe
            return self.forward_train(origin_images)
        else:
            return self.forward_test(origin_images)

    def _loss(self, input, target, return_acc=False):
        logits = self(input)
        loss = self._criterion(logits, target)
        if return_acc:
            acc = accuracy(logits, target)  # use for unrolled acc only (please)
            return loss, acc
        else:
            return loss
