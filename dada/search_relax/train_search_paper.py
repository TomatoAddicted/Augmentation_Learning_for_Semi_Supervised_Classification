...
from dada.search_relax.primitives import sub_policies
#from dada.search_relax.dataset import get_dataloaders, num_class
...
if __name__ == "__main__":
    ...
    parser.add_argument('--gpu', '--gpu_id', type=int, default=0, help='gpu device id')
    ...
    args = get_dataset_params(args, dataset_path)
    ...
    # CIFAR_CLASSES = 10
    CIFAR_CLASSES = args.num_classes #num_class(args.dataset)
...
def avg_op_pos(path, op='Color', epoch=None):
    #  This Function returns the avg position of all occurrences for a given operation
    n = 0  # number of uccurrences
    sum_pos = 0  # sum of all positions
    if epoch is None:
        pass
    else:
    ...
    with open(path, 'r') as f:
        for i, line in enumerate(f):
            if op in line:
                n += 1
                sum_pos += i
    return sum_pos / n  # return avg position of op

def print_genotype(geno, args, run=None, epoch=None):
    # Edited by Pedro: Now also creates txt file with learned sub_policies
    #filename = args.dataset + ('_supervised_augment.txt' if args.nofixmatch else '_fixmatch_augment.txt')
    #augment_file = os.path.join(args.out, filename)
    if epoch is None:
        save_path = args.policy_path
    else:
        save_path = args.policy_path + "_" + str(epoch)
    ...
    for i, sub_policy in enumerate(geno):
        ...
        f.writelines(str(i) + '. ')
        for s in sub_policy:
            f.writelines(s[0] + ': ' + str(s[1:3]) + ' ' + str(s[3]) + ' ')
        f.writelines('\n')
    f.close()
    ...
    if run is not None:
        run["Augmentation weights"].upload_files(save_path)
...
def main():
    ...
    model = Network(
        ...
        device=args.gpu, device_ids=[args.gpu])
    # model = model.cuda()
    ...
    dataloaders = get_dataloaders(args, dataset_path, None)
    train_queue = dataloaders['labeled']
