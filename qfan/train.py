...
from utils.utils import (train, validate, evaluate, build_dataflow, get_augmentor,
                         ...
                         get_mean_and_std, setup_settings_in_yaml, safe_load_image)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Data loading code
    val_augmentor = get_augmentor(False, args.input_size, mean=mean, std=std)

    val_dataset = ImageFolder(os.path.join(args.datadir, args.val_folder),
                              transform=val_augmentor, loader=safe_load_image)
    ...
    if args.evaluate:
        ...
        val_top1, val_top5, val_losses, val_speed = evaluate(val_loader, model, val_criterion, gpu_id=args.gpu, rank=args.rank)
        if args.rank == 0:
    ...
    train_augmentor = get_augmentor(True, args.input_size, mean=mean, std=std)

    train_dataset = ImageFolder(os.path.join(args.datadir, args.train_folder),
                                transform=train_augmentor, loader=safe_load_image)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.rank == 0:
            ...
            summary_writer.add_scalar('best-val-top1', best_top1, epoch + 1)
